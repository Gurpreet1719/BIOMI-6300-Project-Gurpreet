...
# Here, we will add year and treatment as variables
...
phylum_colors <- c(
  ...
  Latescibacterota = "pink1",
  Methylomirabilota = "springgreen2",
  Fibrobacterota = "olivedrab",
  other = "grey")
```
...
phylum_df <- 
  ...
  # fix the order of year
  mutate(year = fct_relevel(year, c("1", "5", "10")),
        treatment = fct_relevel(treatment, c("N0", "N120",
                                          "N180", "N240",
                                          "N360")))
...
family_df <- scaled_rooted_physeq %>%
  ...
  mutate(year = factor(year, levels = c("1", "5", "10")),
         treatment = factor(treatment, levels = c("N0", "N120", "N180", "N240", "N360")))
